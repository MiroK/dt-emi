...
def get_system(u_prev, boundaries, dt, data):
    '''Dispatch'''
    ...
    if elm.family() == 'Lagrange':
        return get_system_cg(u_prev, boundaries, dt, data)

    
def get_system_cg(u_prev, boundaries, dt, data):
    ...
    u1prev, u2prev = u_prev
    Tu1prev, Tu2prev = Trace(u1prev, iface), Trace(u2prev, iface)
    # rhs
    L = block_form(W, 1)
    L[0] = (df.inner(f_vol[0], v1)*df.dx - 
            gamma*df.inner(Tu2prev - Tu1prev, Tv1)*dx_ -
            sum(df.inner(g_robin[tag], Tv1)*dx_(tag) for tag in iface_tags))

    L[1] = (df.inner(f_vol[1], v2)*df.dx +
            gamma*df.inner(Tu2prev - Tu1prev, Tv2)*dx_ + 
...
if __name__ == '__main__':
    ...
    a, L, bcs = get_system(u_prev, boundaries, dt, data=mms_data)
